...
export async function resolveLineOutboundMedia(
  ...
  try {
    ...
    // URL parse failure means this may be a local path; continue to local-path handling.
  }

  if (!opts.mediaBaseUrl?.trim()) {
    throw new Error("LINE local media path requires mediaBaseUrl to be configured");
  }

  void opts.mediaLocalRoots;
  // TODO(PR2): Resolve local media paths to public HTTPS URLs via mediaBaseUrl.
  throw new Error("LINE local media path resolution is not implemented yet (planned for PR2)");
}
